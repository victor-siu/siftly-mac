...
struct DNSProfile: Codable, Identifiable, Equatable, Sendable {
    var id: UUID
    var name: String
    var upstream: [String]
    var fallback: [String]
...
struct SiftlyConfig: Codable, Equatable, Sendable {
...
struct SplitTunnelEntry: Identifiable, Equatable, Sendable {
    let id = UUID()
    var domain: String
    var server: String
...
final class ConfigManager {
    ...
    func saveConfig() {
        ...
        saveConfigTask = Task.detached(priority: .background) {
    ...
    func saveAppState() {
        saveAppStateTask?.cancel()
        let stateCopy = self.appState
        let path = self.appStatePath
        saveAppStateTask = Task.detached(priority: .background) {
